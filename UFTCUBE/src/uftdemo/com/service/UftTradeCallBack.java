...
public class UftTradeCallBack implements ICallBackMethod {
    ...
    private static String returnNumber = ""; //获取报错的返回值

    private static  String returnEntrustNo = ""; //获取第一笔委托的委托no

    private static  String returnEntrustPrice = ""; //获取第一笔委托的委托价格，通过查询委托价格判断是否进行过委托

    private static String returnBuyPrice3=""; //获取400行情买三价格

    public String getReturnEntrustNo(){
        return returnEntrustNo;
    }
    public String getReturnEntrustPrice(){
        return returnEntrustPrice;
    }
    public String getReturnNumber(){
        return returnNumber;
    }

    public String getReturnBuyPrice3(){return returnBuyPrice3;}
    //抓取user_token
...
//抓取sysnode_id
    ...
    public void execute(IEvent arg0, ClientSocket arg1) {
        ...
        if (arg0.getReturnCode() != EventReturnCode.I_OK ) {
            ...
            IDatasets result1 = arg0.getEventDatas();

            IDataset ds1 = result1.getDataset(0);

            returnNumber = ds1.getString(3);
            ...
            logger4j.error(ds1.getString(3));
        ...
        }else if(iFunctionID == 333101){//单独对查询的委托做处理
            //获取结果集
            IDatasets result = arg0.getEventDatas();
            IDataset ds = result.getDataset(0);//里边结果集,这里只获取第一笔
            try {
                returnEntrustNo = ds.getString("entrust_no");
                returnEntrustPrice = ds.getString("entrust_price");
            }catch (Exception e){
                returnEntrustNo="";
                returnEntrustPrice="";
            }
